...
| `SinceId` | `long?` | Optional | The ID of the event from which you want to start the search(ID is not included. e.g. if ID is set to 2, then all events with ID 3 and more will be shown) This parameter is not used if since_date is defined. |
...
| `EventTypes` | [`List<InvoiceEventType>`](../../doc/models/invoice-event-type.md) | Optional | Filter results by event_type. Supply a comma separated list of event types (listed above). Use in query: `event_types=void_invoice,void_remainder`. |
...
ListInvoiceEventsInput listInvoiceEventsInput = new ListInvoiceEventsInput
{
    ...
    WithChangeInvoiceStatus = "with_change_invoice_status2",
    EventTypes = new List<InvoiceEventType>
    {
        InvoiceEventType.ApplyPayment,
        InvoiceEventType.ApplyDebitNote,
    },
};
```
