...
```csharp
ProductsController productsController = client.ProductsController;
```
...
try
{
    ProductResponse result = await productsController.ReadProductByHandleAsync(apiHandle);
}
catch (ApiException e)
{
    // TODO: Handle exception here
...
try
{
    List<ProductResponse> result = await productsController.ListProductsAsync(listProductsInput);
}
catch (ApiException e)
{
    // TODO: Handle exception here
