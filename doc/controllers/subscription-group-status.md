...
```csharp
SubscriptionGroupStatusController subscriptionGroupStatusController = client.SubscriptionGroupStatusController;
```
...
```csharp
CancelDelayedCancellationForGroupAsync(
    string uid)
```
...
try
{
    await subscriptionGroupStatusController.CancelDelayedCancellationForGroupAsync(uid);
}
catch (ApiException e)
{
    // TODO: Handle exception here
...
```csharp
ReactivateSubscriptionGroupAsync(
    string uid,
...
[`Task<Models.ReactivateSubscriptionGroupResponse>`](../../doc/models/reactivate-subscription-group-response.md)

## Example Usage

```csharp
string uid = "uid0";
ReactivateSubscriptionGroupRequest body = new ReactivateSubscriptionGroupRequest
{
    Resume = true,
};
...
try
{
    ReactivateSubscriptionGroupResponse result = await subscriptionGroupStatusController.ReactivateSubscriptionGroupAsync(
        uid,
        body
    );
}
