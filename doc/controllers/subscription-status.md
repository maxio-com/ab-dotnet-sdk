...
```csharp
SubscriptionStatusController subscriptionStatusController = client.SubscriptionStatusController;
```
...
try
{
    SubscriptionResponse result = await subscriptionStatusController.CancelSubscriptionAsync(subscriptionId);
}
catch (ApiException e)
{
    // TODO: Handle exception here
...
```csharp
int subscriptionId = 222;
PauseRequest body = new PauseRequest
{
    Hold = new AutoResume
    {
        AutomaticallyResumeAt = DateTime.ParseExact("2017-05-25T11:25:00Z", "yyyy'-'MM'-'dd'T'HH':'mm':'ss.FFFFFFFK",
...
try
{
    SubscriptionResponse result = await subscriptionStatusController.PauseSubscriptionAsync(
        subscriptionId,
        body
    );
}
...
```csharp
UpdateAutomaticSubscriptionResumptionAsync(
    int subscriptionId,
    Models.PauseRequest body = null)
```
...
PauseRequest body = new PauseRequest
{
    Hold = new AutoResume
    {
        AutomaticallyResumeAt = DateTime.ParseExact("2019-01-20T00:00:00", "yyyy'-'MM'-'dd'T'HH':'mm':'ss.FFFFFFFK",
            provider: CultureInfo.InvariantCulture,
            DateTimeStyles.RoundtripKind),
    },
};
...
try
{
    SubscriptionResponse result = await subscriptionStatusController.UpdateAutomaticSubscriptionResumptionAsync(
        subscriptionId,
        body
    );
}
...
{
    CalendarBilling = new ReactivationBilling
    {
...
try
{
    SubscriptionResponse result = await subscriptionStatusController.ReactivateSubscriptionAsync(
        subscriptionId,
        body
    );
}
...
```csharp
CancelDelayedCancellationAsync(
    int subscriptionId)
```
...
try
{
    DelayedCancellationResponse result = await subscriptionStatusController.CancelDelayedCancellationAsync(subscriptionId);
}
catch (ApiException e)
{
    // TODO: Handle exception here
...
try
{
    SubscriptionResponse result = await subscriptionStatusController.CancelDunningAsync(subscriptionId);
}
catch (ApiException e)
{
    // TODO: Handle exception here
...
[`Task<Models.RenewalPreviewResponse>`](../../doc/models/renewal-preview-response.md)
...
RenewalPreviewRequest body = new RenewalPreviewRequest
{
    Components = new List<Models.RenewalPreviewComponent>
    {
        ...
        {
            ComponentId = RenewalPreviewComponentComponentId.FromNumber(10708),
            Quantity = 10000,
        },
        new RenewalPreviewComponent
        {
            ComponentId = RenewalPreviewComponentComponentId.FromString("handle:small-instance-hours"),
            Quantity = 10000,
            PricePointId = RenewalPreviewComponentPricePointId.FromNumber(8712),
        },
        new RenewalPreviewComponent
        {
            ComponentId = RenewalPreviewComponentComponentId.FromString("handle:large-instance-hours"),
            Quantity = 100,
            PricePointId = RenewalPreviewComponentPricePointId.FromString("handle:startup-pricing"),
        },
    },
};
...
try
{
    RenewalPreviewResponse result = await subscriptionStatusController.PreviewRenewalAsync(
        subscriptionId,
        body
    );
}
