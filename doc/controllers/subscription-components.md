...
```csharp
UpdateSubscriptionComponentsPricePointsAsync(
    int subscriptionId,
...
[`Task<Models.BulkComponentSPricePointAssignment>`](../../doc/models/bulk-component-s-price-point-assignment.md)
...
{
    Components = new List<Models.ComponentSPricePointAssignment>
    {
        new ComponentSPricePointAssignment
        {
            ComponentId = 997,
            PricePoint = ComponentSPricePointAssignmentPricePoint.FromNumber(1022),
        },
        new ComponentSPricePointAssignment
        {
            ComponentId = 998,
            PricePoint = ComponentSPricePointAssignmentPricePoint.FromString("wholesale-handle"),
        },
        new ComponentSPricePointAssignment
        {
            ComponentId = 999,
            PricePoint = ComponentSPricePointAssignmentPricePoint.FromString("_default"),
        },
    },
};
...
try
{
    BulkComponentSPricePointAssignment result = await subscriptionComponentsController.UpdateSubscriptionComponentsPricePointsAsync(
        subscriptionId,
        body
    );
}
...
```csharp
ResetSubscriptionComponentsPricePointsAsync(
    int subscriptionId)
```
...
try
{
    SubscriptionResponse result = await subscriptionComponentsController.ResetSubscriptionComponentsPricePointsAsync(subscriptionId);
}
catch (ApiException e)
{
    // TODO: Handle exception here
...
AllocateComponents body = new AllocateComponents
{
    ...
    Allocations = new List<Models.CreateAllocationRequest>
    {
...
{
    Allocations = new List<Models.CreateAllocation>
    {
...
```csharp
UpdatePrepaidUsageAllocationAsync(
    int subscriptionId,
    int componentId,
    int allocationId,
...
try
{
    await subscriptionComponentsController.UpdatePrepaidUsageAllocationAsync(
        subscriptionId,
        componentId,
        allocationId,
        body
...
string subdomain = "subdomain4";
string apiHandle = "api_handle6";
List<Models.EBBEvent> body = new List<Models.EBBEvent>
{
    new EBBEvent
    {
        Chargify = new ChargifyEBB
...
try
{
    await subscriptionComponentsController.RecordEventsAsync(
        subdomain,
        apiHandle,
        null,
        body
...
| `subscriptionIds` | `List<int>` | Query, Optional | Allows fetching components allocation with matching subscription id based on provided ids. Use in query `subscription_ids=1,2,3`.<br>**Constraints**: *Minimum Items*: `1`, *Maximum Items*: `200` |
| `pricePointIds` | [`IncludeNotNull?`](../../doc/models/include-not-null.md) | Query, Optional | Allows fetching components allocation only if price point id is present. Use in query `price_point_ids=not_null`. |
| `productFamilyIds` | `List<int>` | Query, Optional | Allows fetching components allocation with matching product family id based on provided ids. Use in query `product_family_ids=1,2,3`. |
| `include` | [`ListSubscriptionComponentsInclude?`](../../doc/models/list-subscription-components-include.md) | Query, Optional | Allows including additional data in the response. Use in query `include=subscription`. |
| `filterUseSiteExchangeRate` | `bool?` | Query, Optional | Allows fetching components allocation with matching use_site_exchange_rate based on provided value. Use in query `filter[use_site_exchange_rate]=true`. |
