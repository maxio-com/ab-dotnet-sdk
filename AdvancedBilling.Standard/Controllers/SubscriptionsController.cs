            ...
            => await CreateApiCall<List<Models.SubscriptionResponse>>(ArraySerialization.UnIndexed)
              .RequestBuilder(requestBuilder => requestBuilder
                  ...
                  .Parameters(parameters => parameters
                      ...
                      .Query(query => query.Setup("q", input.Q))
                      .Query(query => query.Setup("q_scope", (input.QScope.HasValue) ? CoreHelper.JsonSerialize(input.QScope.Value).Trim('\"') : null))
                      ...
                      .Query(query => query.Setup("collection_method", (input.CollectionMethod.HasValue) ? CoreHelper.JsonSerialize(input.CollectionMethod.Value).Trim('\"') : null))
                      .Query(query => query.Setup("branding_theme_id", input.BrandingThemeId))
                      ...
                      .Query(query => query.Setup("group_status", (input.GroupStatus.HasValue) ? CoreHelper.JsonSerialize(input.GroupStatus.Value).Trim('\"') : null))
                      .Query(query => query.Setup("dunning_exemption", input.DunningExemption))
                      .Query(query => query.Setup("payment_gateways", input.PaymentGateways))
                      .Query(query => query.Setup("currencies", input.Currencies))
