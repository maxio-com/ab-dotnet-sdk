            ...
            => await CreateApiCall<List<Models.WebhookResponse>>()
              .RequestBuilder(requestBuilder => requestBuilder
                  ...
                  .Parameters(parameters => parameters
                      ...
                      .Query(query => query.Setup("until_date", input.UntilDate))
                      .Query(query => query.Setup("page", input.Page))
                      .Query(query => query.Setup("per_page", input.PerPage))
                      .Query(query => query.Setup("order", (input.Order.HasValue) ? CoreHelper.JsonSerialize(input.Order.Value).Trim('\"') : null))
                      .Query(query => query.Setup("subscription", input.Subscription))))
        ...
        public async Task<Models.EndpointResponse> UpdateEndpointAsync(
            ...
            => await CreateApiCall<Models.EndpointResponse>()
              .RequestBuilder(requestBuilder => requestBuilder
                  ...
                  .Parameters(parameters => parameters
                      .Body(b => b.Setup(body))
                      .Template(template => template.Setup("endpoint_id", endpointId))
                      .Header(header => header.Setup("Content-Type", "application/json"))))
              .ResponseHandler(responseHandler => responseHandler
